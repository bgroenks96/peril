...
public final class DefaultContinentOwnerModel implements ContinentOwnerModel
{
  ...
  public DefaultContinentOwnerModel (final ContinentGraphModel continentGraphModel,
  ...
  public boolean isContinentOwned (final String continentName)
  {
    ...
    if (!continentGraphModel.existsContinentWith (continentName))
    {
      Exceptions.throwIllegalArg ("No continent with name [{}] exists.", continentName);
    }

    final Continent continent = continentGraphModel.continentWith (continentName);
  ...
  public ImmutableSet <ContinentPacket> getContinentsOwnedBy (final Id ownerId)
  {
    ...
    for (final Id continentId : continentGraphModel)
    {
      if (!isContinentOwnedBy (continentId, ownerId)) continue;
      ownedContinents.add (continentGraphModel.continentPacketWith (continentId));
    }
  ...
  private Optional <Id> findOwnerOf (final Id continentId)
  {
    ...
    final Continent continent = continentGraphModel.continentWith (continentId);
