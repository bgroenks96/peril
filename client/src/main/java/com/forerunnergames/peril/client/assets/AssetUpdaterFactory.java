...
public final class AssetUpdaterFactory
{
  ...
  {
    if (AssetSettings.isValidS3BucketPath (AssetSettings.ABSOLUTE_UPDATED_ASSETS_DIRECTORY))
    {
      return new S3AssetUpdater (AssetSettings.ABSOLUTE_UPDATED_ASSETS_DIRECTORY);
    }
    ...
    try
    {
      path = Paths.get (AssetSettings.ABSOLUTE_UPDATED_ASSETS_DIRECTORY);
    }
    catch (final InvalidPathException e)
    {
      throw new RuntimeException (
              Strings.format ("Absolute updated assets directory [{}] is not a valid filesystem path, nor a valid Amazon S3 bucket path.\n\n{}",
                              AssetSettings.ABSOLUTE_UPDATED_ASSETS_DIRECTORY, Throwables.getStackTraceAsString (e)));
    }
